...
import { ArrowLeft, CheckCircle, GitBranch, RotateCcw, BookOpen } from "lucide-react";
...
export default function ReviewPage() {
  ...
  const [stats, setStats] = useState({ cardsReviewed: 0, avgQuality: 0, masteryImprovement: 0 });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([cardsData, nodesData, edgesData]) => {
        const nodeMemoryMap: Record<string, number> = {};
        nodesData.forEach((n: GraphNode) => {
          nodeMemoryMap[n.id] = n.memoryStrength || 0;
        });

        const weakCards = cardsData.filter((card: Flashcard) => {
          const strength = nodeMemoryMap[card.nodeId] || 0;
          return strength < 0.6;
        }).sort((a: Flashcard, b: Flashcard) => {
          return (nodeMemoryMap[a.nodeId] || 0) - (nodeMemoryMap[b.nodeId] || 0);
        });

        setCards(weakCards);
        setNodes(nodesData);
        setEdges(edgesData);
        setMemoryMap(nodeMemoryMap);
        setLoading(false);
      })
      .catch(() => {
        setError("Failed to load review materials");
        setLoading(false);
      });
  }, [documentId]);
  ...
  const handleAnswer = useCallback(async (quality: number) => {
    ...
    setStats(prev => ({
      ...
      avgQuality: ((prev.avgQuality * prev.cardsReviewed + quality) / (prev.cardsReviewed + 1)),
  ...
  if (cards.length === 0) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#050510] text-[#8888AA]">
      <CheckCircle size={48} className="text-[#6BCB77] mb-4" />
      <h2 className="text-2xl font-bold text-[#F0F0FF] mb-2 font-[Space_Grotesk]">All caught up!</h2>
      <p className="mb-6">No cards need review right now.</p>
      <button
        onClick={() => router.push(`/study/${documentId}`)}
        className="bg-[#6C63FF] hover:bg-[#7C73FF] text-white px-6 py-3 rounded-xl font-medium flex items-center gap-2 transition-all"
      >
        <BookOpen size={18} />
        Study All Cards
      </button>
  ...
  return (
    <div className="flex min-h-screen bg-[#050510]">
      <div className="w-1/2 h-screen relative">
        <button
          onClick={() => router.back()}
          className="absolute top-4 left-4 z-10 bg-[#0A0A1F]/80 backdrop-blur border border-[#1E1E3F] rounded-xl px-4 py-2 text-[#F0F0FF] flex items-center gap-2 hover:border-[#A78BFA]/40 transition-all"
        >
          <ArrowLeft size={16} />
          Back
        </button>
      ...
      <div className="w-1/2 h-screen bg-[#0A0A1F] flex flex-col">
        <div className="p-6 border-b border-[#1E1E3F]">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-2xl font-bold text-[#F0F0FF] font-[Space_Grotesk]">Review Mode</h1>
            <div className="text-sm text-[#8888AA]">
              {index + 1} / {cards.length}
            </div>
          </div>
          <Progress value={((index + 1) / cards.length) * 100} className="h-2">
            <ProgressIndicator className="h-full bg-gradient-to-r from-[#A78BFA] to-[#6C63FF]" />
          </Progress>
        </div>
          ...
          <AnimatePresence mode="wait">
            {done ? (
              ...
              >
                ...
                <h2 className="text-3xl font-bold text-[#F0F0FF] mb-4 font-[Space_Grotesk]">Review Complete!</h2>
                <div className="space-y-3 mb-8">
                  <div className="flex justify-between text-sm">
                    <span className="text-[#8888AA]">Cards reviewed</span>
                    <span className="text-[#F0F0FF]">{stats.cardsReviewed}</span>
                  </div>
                  <div className="flex justify-between text-sm">
              ...
              >
                <div
                  className="relative w-full min-h-[300px] bg-gradient-to-br from-[#2D2D4A] to-[#1A1A3A] rounded-2xl p-8 border border-[#1E1E3F] cursor-pointer"
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className="absolute inset-0 backface-hidden bg-gradient-to-br from-[#1E1E3F] to-[#0F0F2E] rounded-2xl p-8 border border-[#1E1E3F]"
